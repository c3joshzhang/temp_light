...
class LoggingCB(MIPCallback):
    ...
    def to_df(self, solve_time, gap, obj):
        if len(self.t) > 0:
            ...
            df = pd.DataFrame(
                np.array([self.t, self.gap, self.obj, self.depth]).T,
                columns=["t", "gap", "bestobj", "depth"],
            )
            ...
            df = pd.DataFrame(
                [[np.ceil(solve_time), np.nan, np.nan, np.nan]],
                columns=["t", "gap", "bestobj", "depth"],
            )
...
def solveIP(
    ...
    if do_phase1:
        ...
        cpx_instance.parameters.mip.tolerances.mipgap.set(
            min([1.0, max([phase1_gap, mipgap])])
        )
    else:
        cpx_instance.parameters.mip.tolerances.mipgap.set(mipgap)
        phase1_time = 0
...
def search(
    ...
    if result:
        ...
        if num_solutions >= 1:
            solutions_matrix = np.zeros(
                (num_solutions, len(cpx_instance.solution.pool.get_values(0)))
            )
            ...
            for sol_idx in range(num_solutions):
                ...
                solutions_matrix[sol_idx] = cpx_instance.solution.pool.get_values(
                    sol_idx
                )
            solutions_obj_matrix = np.concatenate(
                (np.expand_dims(objval_arr, axis=0).T, solutions_matrix), axis=1
            )
...
def cplex_run(args):
    ...
    if prob_name == "setcover":
        ...
        np.random.seed(seed)
        random.seed(seed)
        cpx_instance.objective.set_linear(
            list(
                zip(var_names, np.random.randint(5, 100, len(var_names)).astype(float))
            )
        )
    ...
    my_cb.save_as_npz(
        log_path.joinpath(instance_name + "_log.npz"),
        total_time,
        phase2_gap,
        phase2_bestobj,
    )
...
def main(
    ...
    problems = sorted(
        list(
            filter(
                lambda name: name.endswith(instance_file_type),
                os.listdir(instance_path),
            )
        )
    )[:n_instances]
    solved_probs = [
        name.replace("_pool.npz", instance_file_type)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=60.0,
        help="CPLEX time limit in seconds per instance.",
    )
