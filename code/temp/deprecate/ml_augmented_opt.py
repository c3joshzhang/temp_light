...
def load_model(config, model_dir, data=None):
    ...
    model_name, model, criterion, _, _ = get_model(
        model_dir, var_feature_size, con_feature_size, n_batches=1, **config
    )
...
def get_uncertainty_params(config, model, dt, threshold_type="median"):
    ...
    for graph_idx, data in tqdm(dt):
        ...
        probs, pred, uncertainty, evidence, target, binary_idx = get_prediction(
            config, model, data
        )
        ...
        correct_pred_uncertainty = (
            ...
            else np.mean(uncertainty[target == pred])
        )
    ...
    print(
        "val_u_mean:",
        val_u_mean,
        "val_confident_ratio_mean:",
        val_confident_ratio_mean,
        "val_confident_acc_mean:",
        val_confident_acc_mean,
    )
    print("val_bound_error_stats:")
    print(val_bound_error_stats.describe())
    val_bound_error_stats = val_bound_error_stats.describe()[
        ["min", "mean", "max", "std"]
    ]
...
def reduction_then_solve(
    ...
    if len(confident_idx) > 0:
        assignments = list(
            map(
                lambda ind: cplex.SparsePair([int(ind)], [1]), binary_idx[confident_idx]
            )
        )
    ...
    sol, solve_time, status, gap, bestobj = cplex_solve(
        instance_cpx, timelimit, n_threads=1
    )

    result = dict(
        reduction_solve_time=solve_time,
        reduction_status=status,
        reduction_gap=gap,
        reduction_bestobj=bestobj,
    )
...
def uncertainty_based_reduction(
    instance_cpx,
    solve_timelimit,
    prediction,
    binary_idx,
    ...
    while True:
        confident_idx_ = get_confident_pred_idx(
            uncertainty, prediction, unc_t, binary_idx
        )
    ...
    for i in range(n_iter):
        ...
        confident_idx = get_confident_pred_idx(
            uncertainty, prediction, unc_t, binary_idx
        )
        ...
        reduction_time_limit = (
            solve_timelimit if i == n_iter - 1 else solve_timelimit / 2
        )

        sol, result, log_df = reduction_then_solve(
            instance_cpx, reduction_time_limit, prediction, to_add_idx, binary_idx
        )
...
def node_selection(instance_cpx, sol, timelimit, prediction, binary_idx, var_scores):
    ...
    result = dict(
        ns_solve_time=solve_time, ns_status=status, ns_gap=gap, ns_bestobj=bestobj
    )
...
class AttachDataCB(BranchCallback):
    ...
    def __call__(self):
        ...
        for branch_idx in range(self.get_num_branches()):
            ...
            node_seqnum = self.make_cplex_branch(
                branch_idx, node_data=nodesel_score_child
            )
...
def repair(
    ...
    docpx_instance = get_cplex_instance(
        instance_path, instance_name, interface_type="docplex"
    )
    docpx_instance.add_constraints_(
        [
            docpx_instance.get_var_by_index(i) == prediction[i]
            for i in feasible_confident_idx
        ],
        [str(i) + "_fixed_val" for i in feasible_confident_idx],
    )
    ...
    while repair_timelimit > 0 and len(feasible_confident_idx) > 0:
        ...
        for cname in conflicting_con_names:
            ...
            conflicting_var_idx.update(
                set([var.index for var in constraint.iter_variables()])
            )
        ...
        print(
            f"Repair iter-{repair_iter}: {len(infeasible_fixed_var_idx)} infeasible fixed vars..."
        )
        ...
        print(
            f"# Cons after removing {len(infeasible_fixed_var_idx)} fixed val cuts:",
            docpx_instance.number_of_constraints,
        )
        feasible_confident_idx = list(
            set(feasible_confident_idx) - set(infeasible_fixed_var_idx)
        )
...
def solve_with_warmstart(instance_cpx, timelimit, binary_idx, sol):
    ...
    result = dict(
        iter3_solve_time=solve_time,
        iter3_status=status,
        iter3_gap=gap,
        iter3_bestobj=bestobj,
    )
...
def opt_with_gnn(
    ...
    probs, prediction, uncertainty, evidence, _, binary_idx = get_prediction(
        config, model, data
    )
    ...
    confident_idx = get_confident_pred_idx(
        uncertainty, prediction, uncertainty_threshold, binary_idx
    )
    solution = None
    ...
    if "R" in strategy:
        ...
        if "UPR" in strategy:
            ...
            solution, reduction_result, log_df = reduction_then_solve(
                instance_cpx, reduction_timelimit, prediction, confident_idx, binary_idx
            )
            log_df_lst.append(log_df)
        ...
        if is_infeasible_reduction:
            ...
            is_repaired, feasible_confident_idx, repair_time, repair_iter = repair(
                instance_path,
                instance_name,
                prediction,
                binary_idx,
            ...
            if is_repaired:
                instance_cpx = get_cplex_instance(
                    instance_path, instance_name, interface_type="cplex"
                )
                solution, reduction_result, log_df = reduction_then_solve(
                    instance_cpx,
                    reduction_timelimit,
                    prediction,
                    feasible_confident_idx,
                    binary_idx,
                )
                log_df_lst.append(log_df)
            else:
                print(
                    f">>> {instance_name}: The reduced problem could not be repaired!!!"
                )
        ...
        result["reduction_rate_after_repair"] = (
            len(feasible_confident_idx) / result["num_variables"]
        )
    ...
    if "NS" in strategy:
        ...
        var_scores = (
            probs if config["pred_loss_type"] == "bce" else 1 - (1 / (1 + evidence))
        )

        cb = instance_cpx.register_callback(LoggingCB)
        print(f">>> {instance_name}: Starting NS...")
        solution, ns_result = node_selection(
            instance_cpx, solution, solve_timelimit, prediction, binary_idx, var_scores
        )

        result.update(ns_result)

        cb.save_as_npz(
            str(log_path.joinpath(instance_name + "_log.npz")),
            ns_result["ns_solve_time"],
            ns_result["ns_gap"],
            ns_result["ns_bestobj"],
        )
...
def experiment_opt_with_gnn(args):
    ...
    probs, prediction, uncertainty, evidence, incumbent, binary_idx = get_prediction(
        config, model, data
    )
    ...
    if "R" in strategy:
        ...
        if "UPR" in strategy:
            ...
            sol, reduction_result, log_df = reduction_then_solve(
                instance_cpx, reduction_timelimit, prediction, confident_idx, binary_idx
            )
            log_df_lst.append(log_df)
    ...
    if "NS" in strategy:
        ...
        cb = instance_cpx.register_callback(LoggingCB)
        print(f">>> {instance_name}: Starting NS...")
        sol, ns_result = node_selection(
            instance_cpx, sol, solve_timelimit, prediction, binary_idx, var_scores
        )
    ...
    if not incumbent is None:  # i.e., data is labeled
        ...
        result["confident_acc"] = (
            prediction[confident_idx] == incumbent[confident_idx]
        ).mean()
        result["acc"] = (incumbent == prediction).mean()
        result["avg_pr"] = avg_pr(
            torch.tensor(probs[:, 1]).view(-1), torch.tensor(incumbent.round()).long()
        ).item()
...
def experiment(
    ...
    for i, config in enumerate(model_configs):
        ...
        (
            ...
            val_confident_acc_mean,
            val_bound_error_stats,
        ) = get_uncertainty_params(config, model, val_dt, "median")
        ...
        for target_dt_name in target_dt_names:
            ...
            target_instances = [
                path.split("\\")[-1].removesuffix("_data.pt")
        ...
        for model_idx in range(len(models)):
            ...
            inference_dir = PROJECT_DIR.joinpath(
                "trained_models", config["prob_name"], train_dt_name, "inference"
            )

            for target_dt_idx, target_dt_name in enumerate(target_dt_names):
                print(">>>>>>", target_dt_name)

                instance_path = DATA_DIR.joinpath(
                    "instances", prob_name, target_dt_name
                )
                ...
                for strategy in strategies:
                    ...
                    log_path = inference_dir.joinpath(
                        f"{target_dt_name}_{strategy}_{model_name}"
                    )
                    ...
                    args = pd.DataFrame(
                        ...
                        index=np.arange(len(target_instances)),
                    )
                    args_lst = [
                        deepcopy(model),
                        data_path,
                    ...
                    log_files = [
                        name
                        for name in glob.glob(str(log_path.joinpath("*.csv")))
                        if not "reduction" in name
                    ]

                    if len(log_files) > 0:
                        results_df = pd.concat(
                            [pd.read_csv(file_path) for file_path in log_files]
                        )
