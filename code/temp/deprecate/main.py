...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Index of model configuration at the xlsx files under trained_models folder.",
    )
    parser.add_argument(
        "--solve_t",
        type=float,
        default=1800,
        help="Total time limit in seconds for ml_augmented_opt per instance.",
    )
    parser.add_argument(
        "--reduction_t",
        type=float,
        default=60,
        help="Total time limit in seconds for UPR per instance.",
    )
    parser.add_argument(
        "--strategy", type=str, default="UPR+NS", choices=["UPR+NS", "NS"]
    )
    ...
    for dt_name in [train_dt_name, val_dt_name]:
        ...
        collection_time_limit = (
            solve_timelimit // 2
        )  # It can be also reliably set to 5 mins (for the training and validation datasets in the project)

        cplex_run.main(
            prob_name, dt_name, n_instances, collection_time_limit, n_threads
        )
    ...
    val_dt = get_co_datasets(
        prob_name, [val_dt_name], [config["train_size"]], config["abc_norm"]
    )[0]
    data = val_dt[0][1]
    var_feature_size = data.var_node_features.size(
        -1
    )  # number of features used for variable nodes in the bipartite graph representing MIP
    con_feature_size = data.con_node_features.size(
        -1
    )  # number of features used for constraint nodes in the bipartite graph representing MIP

    model_dir = MODEL_DIR.joinpath(config["prob_name"], train_dt_name)
    model_name, _, _, _, _ = get_model(
        model_dir, var_feature_size, con_feature_size, n_batches=1, **config
    )

    if not model_dir.joinpath(model_name + ".pt").exists():
        train_dt = get_co_datasets(
            prob_name, [train_dt_name], [config["val_size"]], config["abc_norm"]
        )[0]
