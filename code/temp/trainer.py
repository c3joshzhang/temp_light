...
def main(train_dt, val_dt, config, WANDB_LOG):
    ...
    model_dir = PROJECT_DIR.joinpath(
        "trained_models", config["prob_name"], train_dt.dt_name
    )
    ...
    model_name, model, criterion, optimizer, scheduler = get_model(
        model_dir, var_feature_size, con_feature_size, len(train_loader), **config
    )
...
if __name__ == "__main__":
    ...
    data_params = dict(
        random_state=[0], train_size=[1000], val_size=[200], prob_name=[prob_name]
    )
    ...
    for seed in range(1):
        for ID, config in enumerate(param_grid):
            ...
            if ID == 0 or config["abc_norm"] != last_abc_norm_config:
                ...
                train_dt, val_dt = get_co_datasets(
                    prob_name, dt_names, dt_sizes, config["abc_norm"]
                )
