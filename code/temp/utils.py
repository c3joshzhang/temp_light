...
def get_model(
    ...
    **kwargs,
    ...
    criterion = LossHandler(
        edl_lambda, evidence_func, pred_loss_type, n_steps=n_batches * num_epochs
    ).to(DEVICE)
    ...
    if scheduler_type == "step":
        ...
        scheduler = lr_scheduler.ReduceLROnPlateau(
            optimizer, mode="max", factor=0.8, patience=10, min_lr=1e-6
        )
    else:
...
def get_solver_results(solution_path):
    ...
    logs.index = (
        logs.file_name.str.replace(".lp", "", regex=True)
        .str.replace(".mps.gz", "", regex=True)
        .str.replace(".mps", "", regex=True)
    )
