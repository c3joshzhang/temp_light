...
GNN_layers = {
    ...
    "GIN": GIN_bipartite_layers,
    "SAGE": SAGE_bipartite_layers,
}
...
class ErrorLayer(MessagePassing):
    def __init__(self, dim, var_assignment, *args, **kwargs):
        ...
        self.error_encoder = Sequential(
            Linear(1, dim), ReLU(), Linear(dim, dim), ReLU(), BatchNorm(dim)
        )
...
class ViolationLayer(MessagePassing):
    def __init__(self, dim, var_assignment, dropout, activation, norm):
        ...
        self.var_assignment = var_assignment
        self.error_encoder = Sequential(
            Linear(2, dim), self.act1(), Dropout(dropout), Linear(dim, dim)
        )
...
class SimpleMIPGNN(BaseModel):
    def __init__(
        ...
        **kwargs
        ...
        self.var_node_encoder = Sequential(
            PreNormLayer(var_feature_size),
            Linear(var_feature_size, hidden),
            self.act(),
            Linear(hidden, hidden),
        )
        self.con_node_encoder = Sequential(
            PreNormLayer(con_feature_size),
            Linear(con_feature_size, hidden),
            self.act(),
            Linear(hidden, hidden),
        )
        ...
        for i in range(self.num_layers):
            self.layers_con.append(
                GNN_layers[self.gnn_type]["simple"](1, hidden, aggr, 0)
            )
            self.layers_var.append(
                GNN_layers[self.gnn_type]["simple"](1, hidden, aggr, 0)
            )
    ...
    def forward(self, batch):
        ...
        var_batch_idx, con_batch_idx = get_var_and_con_batch_idx(
            batch, num_var_nodes, num_con_nodes, DEVICE
        )
...
class MIPGNN(BaseModel):
    def __init__(
        ...
        **kwargs
        ...
        for i in range(self.num_layers):
            self.layers_ass.append(
                Sequential(
                    Linear(hidden, hidden), self.act(), Linear(hidden, 1), Sigmoid()
                )
            )
        ...
        for i in range(self.num_layers):
            self.layers_con.append(
                GNN_layers[self.gnn_type]["convar"](
                    1, hidden, aggr, 0, activation, norm
                )
            )
            self.layers_var.append(
                GNN_layers[self.gnn_type]["varcon"](
                    1, hidden, self.layers_ass[i], aggr, 0, activation, norm
                )
            )
            self.layers_err.append(
                self.error_layer_cls(hidden, self.layers_ass[i], 0, activation, norm)
            )
