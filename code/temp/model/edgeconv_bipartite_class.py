...
class ECVarConBipartiteLayer(MessagePassing):
    def __init__(
        ...
        self.nn = Sequential(
            Linear(3 * dim + 1, dim), self.act(), Linear(dim, dim), self.act()
        )
    ...
    def forward(self, source, target, edge_index, edge_attr, batch_idx_tuple, size):
        ...
        x = (
            self.norm(x, con_batch_idx)
            if not isinstance(self.norm, BatchNorm)
            else self.norm(x)
        )
        x = self.dropout(x)
...
class ECConVarBipartiteLayer(MessagePassing):
    def __init__(
        ...
        self.nn = Sequential(
            Linear(4 * dim, dim), self.act(), Linear(dim, dim), self.act()
        )
        self.norm = normalizations[norm](dim)
        self.dropout = Dropout(dropout)
...
class ECSimpleBipartiteLayer(MessagePassing):
    def __init__(
        ...
        # Combine node and edge features of adjacent nodes.
        self.nn = Sequential(
            Linear(3 * dim, dim), self.act(), Linear(dim, dim), self.act()
        )
        self.norm = normalizations[norm](dim)
        self.dropout = Dropout(dropout)
    ...
    def forward(self, source, target, edge_index, edge_attr, batch_idx_tuple, size):
        ...
        edge_embedding = self.edge_encoder(edge_attr)
        x = self.propagate(
            edge_index, x=source, t=target, edge_attr=edge_embedding, size=size
        )
        x = (
...
EC_bipartite_layers = {
    "varcon": ECVarConBipartiteLayer,
    "convar": ECConVarBipartiteLayer,
    "simple": ECSimpleBipartiteLayer,
}
