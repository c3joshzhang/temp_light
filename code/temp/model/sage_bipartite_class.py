...
class SAGEVarConBipartiteLayer(MessagePassing):
    def __init__(
        ...
        super(SAGEVarConBipartiteLayer, self).__init__(
        ...
        self.joint_var = Sequential(
            Linear(dim + 1, dim), ReLU(), Linear(dim, dim), ReLU(), BatchNorm(dim)
        )

        # Map edge features to embeddings with the same number of components as node embeddings.
        self.edge_encoder = Sequential(
            Linear(edge_dim, dim), ReLU(), Linear(dim, dim), ReLU(), BatchNorm(dim)
        )
...
class SAGEConVarBipartiteLayer(MessagePassing):
    def __init__(
        ...
        super(SAGEConVarBipartiteLayer, self).__init__(
            aggr=aggr, flow="source_to_target"
        )

        # Learn joint representation of constraint embedding and error.
        self.joint_con = Sequential(
            Linear(dim + dim, dim), ReLU(), Linear(dim, dim), ReLU(), BatchNorm(dim)
        )
...
class SAGESimpleBipartiteLayer(MessagePassing):
    def __init__(
        ...
        super(SAGESimpleBipartiteLayer, self).__init__(
            aggr=aggr, flow="source_to_target"
        )
...
SAGE_bipartite_layers = {
    "varcon": SAGEVarConBipartiteLayer,
    "convar": SAGEConVarBipartiteLayer,
    "simple": SAGESimpleBipartiteLayer,
}
