    ...
    def __init__(
        ...
        if aggr == "comb":
            aggr = MultiAggregation(
                ...
                mode="proj",
                mode_kwargs=dict(in_channels=dim, out_channels=dim),
            )

        super(GINVarConBipartiteLayer, self).__init__(
            aggr=aggr, flow="source_to_target"
        )
        ...
        self.joint_var = Sequential(
            Linear(dim + 1, dim), self.act(), Linear(dim, dim), self.act(), Norm(dim)
        )

        # Map edge features to embeddings with the same number of components as node embeddings.
        self.edge_encoder = Sequential(
            PreNormLayer(edge_dim),
            Linear(edge_dim, dim),
            self.act(),
            Linear(dim, dim),
            self.act(),
        ...
        self.nn = Sequential(
            Linear(dim, dim), self.act(), Linear(dim, dim), self.act(), Norm(dim)
        )
...
class GINConVarBipartiteLayer(MessagePassing):
    def __init__(
        ...
        super(GINConVarBipartiteLayer, self).__init__(
            aggr=aggr, flow="source_to_target"
        )
        ...
        self.joint_con = Sequential(
            Linear(dim * 2, dim), self.act(), Linear(dim, dim), self.act(), Norm(dim)
        )
    ...
    def forward(
        self, source, target, error_con, edge_index, edge_attr, batch_idx_tuple, size
    ):
...
class GINSimpleBipartiteLayer(MessagePassing):
    def __init__(
        ...
        super(GINSimpleBipartiteLayer, self).__init__(
            aggr=aggr, flow="source_to_target"
        )
...
GIN_bipartite_layers = {
    "varcon": GINVarConBipartiteLayer,
    "convar": GINConVarBipartiteLayer,
    "simple": GINSimpleBipartiteLayer,
}
