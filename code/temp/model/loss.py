...
def edl_loss(
    func, y, alpha, epoch_num, num_classes, annealing_step, lambda_, device=DEVICE
):
    y = y.to(DEVICE)
...
def edl_mse_loss(
    ...
    loss, logll, kl_div = mse_loss(
        target, alpha, epoch_num, num_classes, annealing_step, lambda_, device=DEVICE
    )
...
def get_evidence_and_uncertainty(
    ...
    mean_uncertainy_succ = (
        torch.mean(uncertainty[match == 1.0])
        if torch.sum(match == 1.0) > 0
        else torch.tensor(0.5)
    )
    mean_uncertainy_fail = (
        torch.mean(uncertainty[match == 0.0])
        if torch.sum(match == 0.0) > 0
        else torch.tensor(0.5)
    )
    ...
    mean_evidence_fail = torch.sum(total_evidence * (1 - match)) / (
        torch.sum(1 - match) + EPS
    )

    return [
        mean_evidence_succ.item(),
        mean_evidence_fail.item(),
        mean_uncertainy_succ.mean().item(),
        mean_uncertainy_fail.mean().item(),
    ], uncertainty
...
class LossHandler(torch.nn.Module):
    def __init__(self, edl_lambda, evidence_func, loss_type, n_steps):
        ...
        self.evidence_func = (
            evidence_funcs[evidence_func] if evidence_func else exp_evidence
        )
    ...
    def forward(self, global_step, graph_idx, batch, output, y, binary_pred, step_type):
        ...
        if self.loss_type == "bce":
            ...
            loss, A, kl_div = self.loss_func(
                target_output,
                one_hot_embedding(target_y),
                global_step,
                2,
        ...
        evidence_tuple, uncertainty = get_evidence_and_uncertainty(
            target_output,
            discrete_pred.view(-1, 1),
            target_y.view(-1, 1),
            2,
            self.evidence_func,
        )
